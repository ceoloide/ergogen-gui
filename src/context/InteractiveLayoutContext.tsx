...
export type VisualizationMode = 'debug' | 'wireframe' | 'visual';
...
export const InteractiveLayoutProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const state: InteractiveLayoutState = {
    ...
    visualizationMode: storedState?.visualizationMode || 'debug',
  };
  ...
  return (
    <InteractiveLayoutContext.Provider value={{ state, setGrid, setViewState, setActiveTool, setVisualizationMode, resetView, resetAll }}>
      {children}
