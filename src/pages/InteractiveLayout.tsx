...
const InnerLayout = () => {
  ...
  const [cursorPos, setCursorPos] = useState<{ x: number; y: number; snappedX?: number; snappedY?: number }>({ x: 0, y: 0 });

  const handleCursorMove = React.useCallback((x: number, y: number, snappedX?: number, snappedY?: number) => {
    setCursorPos({ x, y, snappedX, snappedY });
  }, []);
