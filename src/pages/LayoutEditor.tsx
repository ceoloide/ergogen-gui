...
const Key = styled.div<{ x: number; y: number; r: number; width: number; height: number; $selected: boolean }>`
  position: absolute;
  left: 0;
  top: 0;
  ...
  box-shadow: ${props => props.$selected ? '0 0 0 2px ' + theme.colors.accent + ', 0 4px 6px rgba(0,0,0,0.3)' : '0 2px 4px rgba(0,0,0,0.2)'};
  cursor: pointer;
...
// Helper to scale generic units to pixels (19mm = 1u approx)
        ...
        <div style={{
            ...
            {showGrid && (
                <div style={{
                    ...
                    `,
                    backgroundSize: `${19 * gridSize * PIXELS_PER_MM}px ${19 * gridSize * PIXELS_PER_MM}px`,
                    opacity: 0.2,
                    pointerEvents: 'none'
                }} />
            )}
            ...
            <div style={{ pointerEvents: 'auto' }}>
                {Object.entries(points).map(([name, point]) => (
                    <Key
                        ...
                        r={-(point.r || 0)}
                        width={19 * PIXELS_PER_MM}
                        height={19 * PIXELS_PER_MM}
