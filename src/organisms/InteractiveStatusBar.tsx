...
interface Props {
  mousePos: { x: number; y: number; snappedX?: number; snappedY?: number };
}
...
const InteractiveStatusBar: React.FC<Props> = ({ mousePos }) => {
  ...
  const displaySnapX = mousePos.snappedX !== undefined ? toDisplay(mousePos.snappedX, grid.displayUnit).toFixed(2) : null;
  const displaySnapY = mousePos.snappedY !== undefined ? toDisplay(mousePos.snappedY, grid.displayUnit).toFixed(2) : null;
  ...
  return (
    <StatusBarContainer>
      ...
      {grid.snapEnabled && displaySnapX && (
        <StatusItem>Snap: {displaySnapX}, {displaySnapY}</StatusItem>
      )}
    </StatusBarContainer>
  );
};
