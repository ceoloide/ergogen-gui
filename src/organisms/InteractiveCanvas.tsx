...
const isKey = (point: any): boolean => {
  const tags = point.tags || point.meta?.tags;
  if (!tags) return false;

  const checkTag = (t: any) => {
      if (typeof t !== 'string') return false;
      return t === 'key' || /^key_.+/.test(t);
  };

  if (typeof tags === 'string') return checkTag(tags);
  if (Array.isArray(tags)) return tags.some(checkTag);
  if (typeof tags === 'object') {
      return Object.keys(tags).some(checkTag);
  }
  return false;
};
  ...
  useEffect(() => {
    ...
    if (results?.points) {
      ...
      points.forEach(p => {
        ...
        if (mode === 'debug') {
            ctx.strokeStyle = isK ? '#ff6d6d' : '#ffffff';
            ctx.lineWidth = 1 / view.k;
            ...
            if (isK) {
                const w = (p.width || 19.05);
                const h = (p.height || 19.05);
