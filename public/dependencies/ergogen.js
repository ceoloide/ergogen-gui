...
(function (global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined'
    ...
    : typeof define === 'function' && define.amd
      ? define(
          ['makerjs', 'js-yaml', 'jszip', 'mathjs', 'kle-serial', 'hull'],
          factory
        )
      : ((global =
  ...
  function (
    ...
    utils.bezier = (points, control_points, accuracy) => {
      ...
      for (let i = 0; i < points.length; i = i + control_points + 1) {
        ...
        const model =
          accuracy >= 0
            ? new m$7.models.BezierCurve(curve_points, accuracy)
            : new m$7.models.BezierCurve(curve_points);
    ...
    var point = class Point {
      ...
      clone() {
        return new Point(this.x, this.y, this.r, u$8.deepcopy(this.meta));
      }
      ...
      unposition(model) {
        return m$6.model.rotate(
          m$6.model.moveRelative(model, [-this.x, -this.y]),
          -this.r
        );
      }
    ...
    var nyc = {
      all: true,
      include: ['src/**/*.js'],
      exclude: ['src/templates/kicad8.js'],
    };
    var require$$8 = {
      name: name,
      version: version$2,
    ...
    const fake_require = (io$1.fake_require = (injection) => (name) => {
      ...
      if (name.endsWith('package.json')) {
        ...
        throw new Error(
          `Unknown dependency "${name}" among the requirements of injection "${injection}"!`
        );
    });
    ...
    prepare$1.inherit = (config) =>
      traverse(config, config, [], (target, key, val, root, breadcrumbs) => {
        if (val && val.$extends !== undefined) {
          ...
          while (candidates.length) {
            ...
            a$7.assert(
              other,
              `"${path}" (reached from "${breadcrumbs.join('.')}.$extends") does not name a valid inheritance target!`
            );
            ...
            a$7.assert(
              !list.includes(other),
              `"${path}" (reached from "${breadcrumbs.join('.')}.$extends") leads to a circular dependency!`
            );
            list.unshift(other);
          }
    ...
    const aggregators = {
      ...
      intersect: (config, name, parts) => {
        ...
        a$5.assert(
          parts.length == 2,
          `Intersect expects exactly two parts, but it got ${parts.length}!`
        );
    ...
    const render_zone = (points._render_zone = (
      ...
      const cols = (zone.columns = a$4.sane(
        zone.columns || {},
      ...
      for (let [col_name, col] of Object.entries(cols)) {
        ...
        a$4.unexpected(col, `points.zones.${zone_name}.columns.${col_name}`, [
          'rows',
          'key',
        ]);
        col.rows = a$4.sane(
    ...
    const perform_autobind = (points._perform_autobind = (points, units) => {
      ...
      const col_lists = {};
      const mirrorzone = (p) =>
        (p.meta.mirrored ? 'mirror_' : '') + p.meta.zone.name;
      ...
      for (const p of Object.values(points)) {
        ...
        if (!bounds[zone][col])
          bounds[zone][col] = { min: Infinity, max: -Infinity };
        if (!col_lists[zone])
          col_lists[zone] = Object.keys(p.meta.zone.columns);
      ...
      for (const p of Object.values(points)) {
        ...
        const bind = (p.meta.bind = a$4.trbl(
          p.meta.bind,
          `${p.meta.name}.bind`,
          -1
        )(units));
    ...
    const complex = (config, name, units, aggregator = _or) => {
      ...
      switch (type) {
        ...
        case 'array':
          ...
          return aggregator(
            config.map((elem) => complex(elem, name, units, alternate))
          );
    ...
    const hull = (config, name, points, outlines, units) => {
      ...
      const concavity = a$2.sane(
        config.concavity || 50,
        `${name}.concavity`,
        'number'
      )(units);
      // Extend should default to `true` if not defined
      const extend = a$2.sane(
        config.extend === undefined || config.extend,
        `${name}.extend`,
        'boolean'
      )(units);
        ...
        (point) => {
          ...
          for (const poly_point of hull_points) {
            ...
            if (extend) {
              ...
              parsed_points.push([
                top_origin[0] + model_origin[0],
                top_origin[1] + model_origin[1],
              ]);
              parsed_points.push([
                top_end[0] + model_origin[0],
                top_end[1] + model_origin[1],
              ]);
              parsed_points.push([
                bottom_origin[0] + model_origin[0],
                bottom_origin[1] + model_origin[1],
              ]);
              parsed_points.push([
                bottom_end[0] + model_origin[0],
                bottom_end[1] + model_origin[1],
              ]);
              ...
              if (w > l) {
                intermediate_points = intermediate_points.concat(
                  m$3.path.toPoints(model.paths.top, 2 + Math.floor(w / l))
                );
                intermediate_points = intermediate_points.concat(
                  m$3.path.toPoints(model.paths.bottom, 2 + Math.floor(w / l))
                );
              }
              if (h > l) {
                intermediate_points = intermediate_points.concat(
                  m$3.path.toPoints(model.paths.left, 2 + Math.floor(h / l))
                );
                intermediate_points = intermediate_points.concat(
                  m$3.path.toPoints(model.paths.right, 2 + Math.floor(h / l))
                );
              }
              for (let i = 0; i < intermediate_points.length; i++) {
                const p = intermediate_points[i];
                if (
                  !m$3.measure.isPointEqual(p, top_origin) &&
    ...
    const path = (config, name, points, outlines, units) => {
      ...
      for (const [index, segment] of segments.entries()) {
        a$2.in(segment.type, `${name}.segments.${index}.type`, [
          'line',
          'arc',
          's_curve',
          'bezier',
        ...
        switch (segment.type) {
          case 'bezier':
            a$2.unexpected(segment, `${name}.segments.${index}`, [
              'type',
              `points`,
              'accuracy',
            ]);
        ...
        switch (segment.type) {
          case 'bezier':
            a$2.assert(
              num_points > (index === 0 ? 2 : 1),
              `Bezier Curve needs 3 or 4 points, but ${num_points} were provided`
            );
            break;
          case 'arc':
            a$2.assert(
              num_points === (index === 0 ? 3 : 2),
              `Arc needs 3 points, but ${num_points} ${num_points === 1 ? 'was' : 'were'} provided`
            );
            break;
          case 'line':
            a$2.assert(
              num_points > (index === 0 ? 1 : 0),
              `Line need at least 2 points, but ${num_points} ${num_points === 1 ? 'was' : 'were'} provided`
            );
            break;
          case 's_curve':
            a$2.assert(
              num_points === (index === 0 ? 2 : 1),
              `S-Curve needs 2 points, but ${num_points} ${num_points === 1 ? 'was' : 'were'} provided`
            );
            break;
        }
      }
      ...
      return [
        (point) => {
            ...
            switch (segment.type) {
              ...
              case 's_curve':
                ...
                a$2.assert(
                  ...
                  'The ${name}.segments.${index} S-Curve segment cannot have points on the same X axis'
                );
                ...
                a$2.assert(
                  ...
                  'The ${name}.segments.${index} S-Curve segment cannot have points on the same Y axis'
                );
                ...
                const mirrored_s_curve = m$3.model.mirror(
                  s_curve_raw,
                  mirrorX,
                  mirrorY
                );
          ...
          if (
            first_anchor.x !== last_anchor.x ||
            first_anchor.y != last_anchor.y
          ) {
            let closing_line = new m$3.paths.Line(
              [first_anchor.x, first_anchor.y],
              [last_anchor.x, last_anchor.y]
            );
          ...
          a$2.assert(
            chain.endless,
            "The provided path configuration doesn't generate a closed shape."
          );
    ...
    outlines.parse = (config, points, units) => {
      ...
      for (let [outline_name, parts] of Object.entries(config)) {
        ...
        for (let [part_name, part] of Object.entries(parts)) {
          ...
          const operation =
            u$2[
              a$2.in(part.operation || 'add', `${name}.operation`, [
                'add',
                'subtract',
                'intersect',
                'stack',
          ...
          const what = a$2.in(part.what || 'outline', `${name}.what`, [
            'rectangle',
            'circle',
            'polygon',
            'outline',
          ...
          const where = (units) =>
            filter$1(original_where, `${name}.where`, points, units, asym);
          ...
          const joints = a$2.in(
            a$2.sane(part.joints || 0, `${name}.joints`, 'number')(units),
            `${name}.joints`,
            [0, 1, 2]
          );
          ...
          if (scale !== 1) {
            outlines[outline_name] = m$3.model.scale(
    ...
    cases.parse = (config, outlines, units) => {
      ...
      for (let [case_name, case_config] of Object.entries(cases_config)) {
        ...
        for (let [part_name, part] of Object.entries(parts)) {
          ...
          const what = a$1.in(part.what || 'outline', `${part_qname}.what`, [
            'outline',
            'case',
          ]);
          ...
          if (what == 'outline') {
            ...
            const extruded_name =
              `${name}_extrude_` + ('' + extrude).replace(/\D/g, '_');
  ...
  (segment (start ${p.eaxy(5.08, 16.7)}) (end ${p.eaxy(5.08, 14.95)}) (width ${p.signal_trace_width}) (layer "B.Cu") (net ${socket_nets[4].index}))
    ...
    var display_ssd1306 = {
      ...
      body: (p) => {
        let dst_nets = [p.SDA, p.SCL, p.VCC, p.GND];
    ...
    var utility_keepout_zone = {
      ...
      body: (p) => {
        if (p.hatch_pitch < 0 || p.hatch_pitch > 2) {
          throw new Error(
            'Parameter hatch_pitch must be a positive number below 2mm to avoid KiCad issues. Current value: ' +
              p.hatch_pitch +
              'mm'
          );
        }
  ...
  (footprint "ceoloide:utility_point_debugger"
    ...
    (fp_line (start 0 -0.6) (end -0.1 -0.5) (layer "Dwgs.User") (stroke (width 0.05) (type solid)))
      ...
      `;
        ...
        if ('point' in p) {
          // Only available from commit #852c100
          final += ` 
        ...
        const parse_tuple = (t) => {
          let str_tuple = JSON.parse(
            t
              ...
              .replace(/[,]*\)/g, ']')
          ); // replace closing parenthesis (and any trailing comma's) with a bracket
        ...
        const get_traces = (route, net) => {
          ...
          for (let i = 0; i < route.length; i++) {
            ...
            switch (command) {
              ...
              default:
                throw new Error(
                  `Unsupported character '${command}' at position ${i}.`
                );
            }
          }
  ...
  (gr_text "${text}"
      ...
      body: (p) => {
        const gen_3d_model = (
          ...
          const final_rotation =
            rotation || (is_front ? rotation_f : rotation_b);
    ...
    var conn_molex_pico_ezmate_1x02 = {
      params: {
        ...
        cable_3dmodel_filename:
          '${EG_INFUSED_KIM_3D_MODELS}/Molex_Ezmate_Pico_Cable_2pin.step',
        cable_3dmodel_side: '',
        ...
        socket_3dmodel_filename:
          '${EG_INFUSED_KIM_3D_MODELS}/Molex_Ezmate_Pico_Socket_2pin.step',
        socket_3dmodel_side: '',
    ...
    var conn_molex_pico_ezmate_1x05 = {
      params: {
        ...
        cable_3dmodel_filename:
          '${EG_INFUSED_KIM_3D_MODELS}/Molex_Ezmate_Pico_Cable_5pin.step',
        cable_3dmodel_side: '',
        cable_3dmodel_xyz_scale: '',
        cable_3dmodel_xyz_rotation: '',
        cable_3dmodel_xyz_offset: '',
        ...
        socket_3dmodel_filename:
          '${EG_INFUSED_KIM_3D_MODELS}/Molex_Ezmate_Pico_Socket_5pin.step',
        socket_3dmodel_side: '',
        socket_3dmodel_xyz_scale: '',
        socket_3dmodel_xyz_rotation: '',
        socket_3dmodel_xyz_offset: '',
    ...
    var diode = {
      params: {
        ...
        diode_3dmodel_filename:
          '${EG_INFUSED_KIM_3D_MODELS}/Diode_1N4148W.step',
        diode_3dmodel_side: '',
    ...
    var nice_nano_pretty = {
      params: {
        ...
        header_3dmodel_filename:
          '${EG_INFUSED_KIM_3D_MODELS}/PinHeader_2.54mm_2x-12.step',
        header_3dmodel_xyz_scale: '',
        header_3dmodel_xyz_rotation: '',
        header_3dmodel_xyz_offset: '',

        socket_3dmodel_filename:
          '${EG_INFUSED_KIM_3D_MODELS}/PinSocket_2.54mm_5mm_2x-12.step',
        socket_3dmodel_xyz_scale: '',
        socket_3dmodel_xyz_rotation: '',
        socket_3dmodel_xyz_offset: '',
      },
    ...
    var nice_view = {
      params: {
        ...
        header_3dmodel_filename:
          '${EG_INFUSED_KIM_3D_MODELS}/PinHeader_2.54mm_1x-5.step',
        header_3dmodel_xyz_scale: '',
        header_3dmodel_xyz_rotation: '',
        header_3dmodel_xyz_offset: '',

        socket_3dmodel_filename:
          '${EG_INFUSED_KIM_3D_MODELS}/PinSocket_2.54mm_5mm_1x-5.step',
        socket_3dmodel_xyz_scale: '',
        socket_3dmodel_xyz_rotation: '',
        socket_3dmodel_xyz_offset: '',
      },
      ...
      `;
        ...
        let dst_nets = [p.MOSI.str, p.SCK.str, p.VCC.str, p.GND.str, p.CS.str];
        local_nets = [
          p.local_net('1').str,
          p.local_net('2').str,
          p.VCC.str,
    ...
    var smd_0805 = {
      ...
      body: (p) => {
        ...
        const gen_nets = (p) => {
          ...
          if (
            pad_cnt > all_nets_from.length ||
            pad_cnt > all_nets_to.length ||
            pad_cnt > all_labels.length
          ) {
            pad_cnt = Math.min(
              all_nets_from.length,
              all_nets_to.length,
              all_labels.length
            );
          }
          ...
          let nets = [];
          for (let i = 0; i < pad_cnt; i++) {
            let net = [all_nets_from[i], all_nets_to[i], all_labels[i]];
            nets.push(net);
          }
    ...
    var switch_power = {
      params: {
        ...
        switch_3dmodel_filename:
          '${EG_INFUSED_KIM_3D_MODELS}/Switch_Power.step',
        switch_3dmodel_side: '',
        switch_3dmodel_xyz_scale: '',
        switch_3dmodel_xyz_rotation: '',
        switch_3dmodel_xyz_offset: '',
    ...
    var switch_reset = {
      params: {
        ...
        switch_3dmodel_filename:
          '${EG_INFUSED_KIM_3D_MODELS}/Switch_Reset.step',
        switch_3dmodel_side: '',
        switch_3dmodel_xyz_scale: '',
        switch_3dmodel_xyz_rotation: '',
        switch_3dmodel_xyz_offset: '',
    ...
    var trackpoint_mount = {
      params: {
        ...
        tp_cap_3dmodel_filename:
          '${EG_INFUSED_KIM_3D_MODELS}/trackpoint/TP_Cap_Red_T460S.step',
        tp_cap_3dmodel_xyz_scale: '',
        tp_cap_3dmodel_xyz_rotation: '',
        tp_cap_3dmodel_xyz_offset: '',

        tp_extension_3dmodel_filename:
          '${EG_INFUSED_KIM_3D_MODELS}/trackpoint/TP_Extension_Red_T460S_h10.5_md0.0_pcb1.6.step',
        tp_extension_3dmodel_xyz_scale: '',
        tp_extension_3dmodel_xyz_rotation: '',
        tp_extension_3dmodel_xyz_offset: '',

        tp_3dmodel_filename:
          '${EG_INFUSED_KIM_3D_MODELS}/trackpoint/TP_Red_T460S_platform_z_offset_+0.0_pcb_offset_-2.0.step',
    ...
    var footprints = {
      ...
      'ceoloide/utility_keepout_zone': utility_keepout_zone,
    ...
    var kicad5 = {
        ...
        m$1.model.walk(model, {
          onPath: (wp) => {
            ...
            switch (p.type) {
              case 'line':
                grs.push(
                  `(gr_line (start ${xy(p.origin)}) (end ${xy(p.end)}) (angle 90) (layer ${layer}) (width 0.15))`
                );
                break;
              case 'arc':
                const arc_center = p.origin;
                const angle_start =
                  p.startAngle > p.endAngle ? p.startAngle - 360 : p.startAngle;
                const angle_diff = Math.abs(p.endAngle - angle_start);
                const arc_end = m$1.point.rotate(
                  m$1.point.add(arc_center, [p.radius, 0]),
                  angle_start,
                  arc_center
                );
                grs.push(
                  `(gr_arc (start ${xy(arc_center)}) (end ${xy(arc_end)}) (angle ${-angle_diff}) (layer ${layer}) (width 0.15))`
                );
                break;
              case 'circle':
                ...
                grs.push(
                  `(gr_circle (center ${xy(circle_center)}) (end ${xy(circle_end)}) (layer ${layer}) (width 0.15))`
                );
                break;
              default:
                throw new Error(
      ...
      body: (params) => {
        ...
        const netclass_text = params.nets
          .map((net) => `(add_net "${net.name}")`)
          .join('\n');
        ...
        m.model.walk(model, {
          onPath: (wp) => {
            ...
            switch (p.type) {
              case 'line':
                grs.push(
                  `(gr_line (start ${xy(p.origin)}) (end ${xy(p.end)}) (layer ${layer}) (stroke (width 0.15) (type default)))`
                );
                break;
              case 'arc':
                const arc_center = p.origin;
                const angle_start =
                  p.startAngle > p.endAngle ? p.startAngle - 360 : p.startAngle;
                const angle_diff = Math.abs(p.endAngle - angle_start);
                const arc_start = m.point.rotate(
                  m.point.add(arc_center, [p.radius, 0]),
                  angle_start,
                  arc_center
                );
                const arc_mid = m.point.rotate(
                  arc_start,
                  angle_diff / 2,
                  arc_center
                );
                ...
                grs.push(
                  `(gr_arc (start ${xy(arc_start)}) (mid ${xy(arc_mid)}) (end ${xy(arc_end)}) (layer ${layer}) (stroke (width 0.15) (type default)))`
                );
                break;
              case 'circle':
                const circle_center = p.origin;
                ...
                grs.push(
                  `(gr_circle (center ${xy(circle_center)}) (end ${xy(circle_end)}) (layer ${layer}) (stroke (width 0.15) (type default)) (fill none))`
                );
                break;
              default:
                throw new Error(
      ...
      (config, name, point) => {
        ...
        for (const [param_name, param_def] of Object.entries(fp.params)) {
          ...
          const converters = {
            string: (v) => v,
            number: (v) =>
              a.sane(v, `${name}.params.${param_name}`, 'number')(units),
    ...
    pcbs.parse = (config, points, outlines, units) => {
      ...
      for (const [pcb_name, pcb_config] of Object.entries(pcbs)) {
        ...
        for (const [outline_name, outline] of Object.entries(config_outlines)) {
          ...
          kicad_outlines[outline_name] = template.convert_outline(
            outlines[ref],
            layer
          );
        }
